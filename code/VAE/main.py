...
def Show_Results(Recon_Losses, KLdiv_Losses, img_list):
	...
	Total_losses = list(np.sum([Recon_Losses, KLdiv_Losses], axis=0))
...
def Train():
	...
	for epoch in range(config.num_epochs):
		...
		for i, data in enumerate(dataloader, 0):
			...
			epsilons = torch.normal(0, 1, size=[batch_size, config.nz])
			...
			if i % 50 == 0:
				print('[%d/%d][%d/%d]\tTotal Loss: %.4f\tRecons. Loss: %.4f\tKLDiv. Loss: %.4f\t'
					  % (epoch, config.num_epochs, i, len(dataloader),
						 loss.item(), recon_loss.item(), KLDiv_loss.item()))
	...
	return Recon_Losses, KLdiv_Losses, img_list
...
if __name__ == '__main__':
	...
	Recon_Losses, KLdiv_Losses, img_list = Train()
	Show_Results(Recon_Losses, KLdiv_Losses, img_list)
	Save_Models()
